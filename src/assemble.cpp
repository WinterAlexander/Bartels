    ...
    for (unsigned int ie = 0; ie < E_from.rows(); ++ie) {
        ...
        for (unsigned int jj = 0; jj < tmp.cols(); ++jj) {
            for (unsigned int ii = 0; ii < tmp.rows(); ++ii) {
                triplet_list.push_back(Eigen::Triplet<Scalar>(
                        block_size_to * E_to(ie, ii / block_size_to) + ii % block_size_to,
    ...
    for (unsigned int ie = 0; ie < E_from.rows(); ++ie) {
        ...
        for (unsigned int iblock_to = 0; iblock_to < E_to.cols(); ++iblock_to) {
            for (unsigned int iblk_r = 0; iblk_r < block_size_to; ++iblk_r) {
                ...
                assembled(block_size_to * E_to(ie, iblock_to) + iblk_r) += tmp(
                        block_size_to * iblock_to + iblk_r);
            }
        }
    }
